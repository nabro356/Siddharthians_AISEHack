...
def _eval_threshold_mandal(disease_data, rule, ref_date):
    """
    Check if any mandal has ≥ min_cases in the last window_weeks.
    Returns list of triggered alerts (one per mandal).
    """
    alerts = []
    ...
    mandal_counts = recent.groupby("mandal").size().reset_index(name="cases")
    ...
    for _, row in triggered.iterrows():
        alerts.append({
            "mandal": row["mandal"],
            "cases": int(row["cases"]),
            "detail": f"{row['cases']} cases in {row['mandal']} (threshold: {min_cases})",
        })
    return alerts
...
def _eval_surge(ts_weekly, rule, disease_key):
    ...
    if baseline_mean > 0 and latest["case_count"] >= multiplier * baseline_mean:
        ...
        alerts.append({
            ...
            "detail": f"Weekly cases ({int(latest['case_count'])}) = {ratio:.1f}× baseline avg ({baseline_mean:.1f})",
        })
    return alerts
...
def evaluate_rules(df_clean, ts_weekly, ts_mandal_weekly=None, ref_date=None):
    ...
    for disease_key, disease_info in DISEASE_CODES.items():
        ...
        for rule in rules:
            ...
            if rule_type == "threshold_mandal":
                triggered = _eval_threshold_mandal(disease_data, rule, ref_date)
    ...
    if not all_alerts:
        return pd.DataFrame(columns=[
            ...
            "description", "mandal", "cases", "detail", "timestamp",
        ])
