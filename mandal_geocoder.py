...
def build_mandal_lookup(df: pd.DataFrame, save_path: str = LOOKUP_FILE) -> pd.DataFrame:
    ...
    if unmapped:
        print(f"\n  WARNING: {len(unmapped)} mandals in target diseases have NO geocode. Initiating API Fallback...")
        
        fallback_rows = []
        for m in sorted(unmapped):
            count = (target[mandal_col] == m).sum()
            print(f"    - Fetching {m} ({count} records)... ", end="")
            
            # Find the district for this mandal to make the query accurate
            districts_for_m = target[target[mandal_col] == m][district_col].dropna().unique()
            d_name = districts_for_m[0] if len(districts_for_m) > 0 else "Andhra Pradesh"
            
            # API Query
            query = f"{m} Mandal, {d_name}, Andhra Pradesh, India"
            safe_query = urllib.parse.quote(query)
            url = f"https://nominatim.openstreetmap.org/search?q={safe_query}&format=json"
            
            req = urllib.request.Request(url, headers={'User-Agent': 'AP-RTGS-Dashboard/1.0'})
            try:
            ...
            time.sleep(1.2) # Courteous delay for free API
            
        if fallback_rows:
            new_df = pd.DataFrame(fallback_rows)
            lookup = pd.concat([lookup, new_df], ignore_index=True)
            lookup.to_csv(save_path, index=False)
            print(f"\n  ✓ Saved {len(fallback_rows)} new API-fetched mandals to {save_path}!")
